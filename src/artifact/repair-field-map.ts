...
import { inferFieldMap, isNegatedSponsorshipQuestion, buildHeuristicFields, heuristicFieldFromControl, pickPath } from './infer-field-map.js';

export { isNegatedSponsorshipQuestion, inferFieldMap, buildHeuristicFields };
...
export async function repairFieldMap(opts: {
  ...
  const heuristicFields = inferFieldMap(controls, opts.profileKeys);
...
export function overlayHeuristicsFromControls(
  ...
  const more = inferFieldMap(controls, profileKeys);
